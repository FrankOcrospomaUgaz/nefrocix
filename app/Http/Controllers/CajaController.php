...
class CajaController extends Controller
{
    ...
    public function pdfDetalleCierreExcel(Request $request) {
        ...
        Excel::create('ExcelReporte', function($excel) use($nomcierre2, $listaventas, $listacuotas, $listaventasfarmacia, $listaingresosvarios, $listaegresos, $listacajaanterior, $listaegresoscompra, $nomcierre, $responsable, $caja, $request) {
            ...
            $excel->sheet("Det. Cierre " . $nomcierre, function($sheet) use($nomcierre2, $listaventas, $listacuotas, $listaventasfarmacia, $listaingresosvarios, $listaegresos, $listacajaanterior, $listaegresoscompra, $nomcierre, $responsable, $caja, $request) {
                ...
                $sheet->getRowDimension(4)->setRowHeight(300);
    ...
    public function pdfDetalleCierre(Request $request){
       ...
       /* $sucursal_id = Session::get('sucursal_id');
        ...
        if(count($listaingresosvarios)>0){
            ...
            foreach ($listaingresosvarios as $row) { 
                $pdf::SetFont('helvetica','',6);                   
                $pdf::Cell(15,7,utf8_decode(date("d-m-Y", strtotime($row['fecha']))),1,0,'C');
                $pdf::Cell(56,7,$row['paciente'],1,0,'L');
                $pdf::Cell(20,7,$row['numero'],1,0,'C');
                //$pdf::Cell(12,7,utf8_decode($row['voucher']),1,0,'C');
                $pdf::Cell(134,7,$row['nombre'].': '.$row['comentario'],1,0,'L');
                if($row['situacion'] == 'N') {
                    $valuetp = number_format($row['totalpagado'],2,'.','');
                    $valuetpv = number_format($row['totalpagadovisa'],2,'.','');
                    $valuetpm = number_format($row['totalpagadomaster'],2,'.','');
        ...
        if(count($listaegresos)>0){
            ...
            foreach ($listaegresos as $row) { 
                ...
                $pdf::Cell(15,7,utf8_decode(date("d-m-Y", strtotime($row['fecha']))),1,0,'C');
                if($row['ruc'] == null){
                    $pdf::Cell(56,7,$row->responsable->nombres." ".$row->responsable->apellidopaterno." ".$row->responsable->apellidomaterno,1,0,'L');
                }else{
                    $pdf::Cell(56,7,$row['razonsocial'],1,0,'L');
                }
                $pdf::Cell(20,7,($row->tipodocumento==NULL?"":$row->tipodocumento->abreviatura)." ".$row['numero']." ".$row['numeroserie2'],1,0,'C');
                //$pdf::Cell(12,7,utf8_decode($row['voucher']),1,0,'C');
                $pdf::Cell(134,7,substr($row['nombre'].': '.$row['comentario'], 0, 95),1,0,'L');
                if($row['situacion'] == 'N') {
    ...
    public function pdfRecibo(Request $request){
        ...
        if($lista->conceptopago_id==10){//GARANTIAS
            ...
            if($lista->doctor_id>0){
                $pdf::SetFont('helvetica','B',9);
                $pdf::Cell(20,7,utf8_decode("MÉDICO :"),0,0,'L');
                $pdf::SetFont('helvetica','',9);
    ...
    public function reporteCiruProConDetalle(Request $request) {
        ...
        $pdf::Cell(15,7,utf8_decode("DNI/CE"),1,0,'C');
        $pdf::Cell(30,7,utf8_decode("MÉDICO"),1,0,'C');
...
*/
    ...
    public function pdfDetallePorProducto(Request $request){
        ...
        $pdf::Cell(31,7,utf8_decode("MÉDICO"),1,0,'C');
        $pdf::Ln();/*
    ...
    public function pdfReportePagoOjos(Request $request) {
        ...
        $pdf = new TCPDF();
        $pdf::SetTitle('Reporte de Pago a Médicos Oftalmologia ' . $fechainicial . " - " . $fechafinal);
        $pdf::AddPage('L', 'A4');

        $pdf::SetFont('helvetica','B',15);
        $pdf::Cell(0,10,'REPORTE DE PAGO A MÉDICOS OFTALMOLOGIA: ' . $fechainicial . " - " . $fechafinal,0,0,'C');
        $pdf::Ln();

        $pdf::SetFont('helvetica','B',8);

        $pdf::Cell(20,10,utf8_decode("FECHA"),1,0,'C');
        $pdf::Cell(70,10,utf8_decode("MÉDICO"),1,0,'C');
    ...
    public function pdfReportePago(Request $request) {
        ...
        $pdf = new TCPDF();
        $pdf::SetTitle('Reporte de Pago a Médicos Especialidades ' . $fecha1);
        $pdf::AddPage('L', 'A4');

        $pdf::SetFont('helvetica','B',15);
        $pdf::Cell(0,10,'REPORTE DE PAGO A MÉDICOS ESPECIALIDADES: ' . $fecha1,0,0,'C');
        $pdf::Ln();
