...
<table id="example1" class="table table-bordered table-striped table-condensed table-hover">
	...
	<tbody>
		...
		<tr @if($value->situacion=='A') style="background-color:#FFD0C5;" @endif>
			...
			<td>{{ $value->numero }}</td>
			<td>{{ ($value->documento_id==''?'-':$value->documento_id) }}</td>
			<td>{{ ($value->documento_num==''?'-':$value->documento_num) }}</td>
			<td>{{ ($value->documento_rs==''?'-':$value->documento_rs) }}</td>
			<td>{{ ($value->documento_fecha_rs==''?'-':date("d/m/Y",strtotime($value->documento_fecha_rs))) }}</td>
