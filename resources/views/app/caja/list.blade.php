...
@if($conceptopago_id==1) 
	{!! Form::button('<i class="glyphicon glyphicon-plus"></i> Apertura', array('class' => 'btn-sm btn btn-info', 'disabled' => 'true', 'id' => 'btnApertura')) !!}
    {!! Form::button('<i class="glyphicon glyphicon-usd"></i> Nuevo', array('class' => 'btn-sm btn btn-success', 'id' => 'btnCerrar', 'onclick' => 'modalCaja (\''.URL::route($ruta["create"], array('listar'=>'SI')).'\', \''.$titulo_registrar.'\', this);')) !!}
    {!! Form::button('<i class="glyphicon glyphicon-remove-circle"></i> Cierre', array('class' => 'btn-sm btn btn-danger', 'id' => 'btnCerrar', 'onclick' => 'modalCaja (\''.URL::route($ruta["cierre"], array('listar'=>'SI')).'\', \''.$titulo_cierre.'\', this);')) !!}    

       
@if(($tipousuario!=11&&$user->sucursal_id==1)||$user->usertype_id==1)
    <!--{!! Form::button('<i class="glyphicon glyphicon-list"></i> Tickets Pendientes', array('class' => 'btn-sm btn btn-primary', 'id' => 'btnTicketsPendientes', 'onclick' => 'modalCaja (\''.URL::route($ruta["ticketspendientes"], array('listar'=>'SI')).'\', \''.$titulo_ticketspendientes.'\', this);')) !!}
    {!! Form::button('<i class="glyphicon glyphicon-tags"></i>&nbsp;&nbsp;Cuentas por Cobrar', array('class' => 'btn-sm btn btn-success', 'id' => 'btnCuentasPendientes', 'onclick' => 'modalCaja (\''.URL::route($ruta["cuentaspendientes"], array('listar'=>'SI')).'\', \''.$titulo_cuentaspendientes.'\', this);')) !!}-->
@endif

@if(($tipousuario!=11&&$user->sucursal_id==1)||$user->usertype_id==1 && $sucursal_id == 1)
    <!--{!! Form::button('<i class="glyphicon glyphicon-link"></i>&nbsp;&nbsp;Pagos pendientes a doctores', array('class' => 'btn-sm btn btn-danger', 'id' => 'btnpagodoc', 'onclick' => 'modal("caja/pagosdoctoresojos", "Pagos pendientes a doctores");')) !!}-->
@endif 

@if($user->sucursal_id==2||$user->usertype_id==1 && $sucursal_id == 2)
    <!--{!! Form::button('<i class="glyphicon glyphicon-link"></i>&nbsp;&nbsp;Pagos pendientes a doctores', array('class' => 'btn-sm btn btn-danger', 'id' => 'btnpagodoc', 'onclick' => 'modal("caja/pagosdoctores", "Pagos pendientes a doctores");')) !!}-->
@endif


@elseif($conceptopago_id==2)
    {!! Form::button('<i class="glyphicon glyphicon-plus"></i> Apertura', array('class' => 'btn-sm btn btn-info', 'id' => 'btnApertura')) !!}
    {!! Form::button('<i class="glyphicon glyphicon-usd"></i> Nuevo', array('class' => 'btn-sm btn btn-success', 'disabled' => 'true', 'id' => 'btnCerrar', 'onclick' => 'modalCaja (\''.URL::route($ruta["create"], array('listar'=>'SI')).'\', \''.$titulo_registrar.'\', this);')) !!}
    {!! Form::button('<i class="glyphicon glyphicon-remove-circle"></i> Cierre', array('class' => 'btn-sm btn btn-danger' , 'disabled' => 'true', 'id' => 'btnCerrar', 'onclick' => 'modalCaja (\''.URL::route($ruta["cierre"], array('listar'=>'SI')).'\', \''.$titulo_cierre.'\', this);')) !!}
@else
    {!! Form::button('<i class="glyphicon glyphicon-plus"></i> Apertura', array('class' => 'btn-sm btn btn-info', 'disabled' => 'true', 'id' => 'btnApertura', 'onclick' => 'modalCaja (\''.URL::route($ruta["apertura"], array('listar'=>'SI')).'\', \''.$titulo_apertura.'\', this);')) !!}
    {!! Form::button('<i class="glyphicon glyphicon-usd"></i> Nuevo', array('class' => 'btn-sm btn btn-success', 'id' => 'btnCerrar', 'onclick' => 'modalCaja (\''.URL::route($ruta["create"], array('listar'=>'SI')).'\', \''.$titulo_registrar.'\', this);')) !!}
    {!! Form::button('<i class="glyphicon glyphicon-remove-circle"></i> Cierre', array('class' => 'btn-sm btn btn-danger', 'id' => 'btnCerrar', 'onclick' => 'modalCaja (\''.URL::route($ruta["cierre"], array('listar'=>'SI')).'\', \''.$titulo_cierre.'\', this);')) !!}
...
@endif
<!--{! Form::button('<i class="glyphicon glyphicon-print"></i> Honorario', array('class' => 'btn-sm btn btn-warning', 'id' => 'btnHonorario', 'onclick' => 'imprimirHonorario();')) !!}-->

{!! Form::button('<i class="glyphicon glyphicon-print"></i> Exportar PDF', array('class' => 'btn-sm btn btn-warning', 'id' => 'btnDetalle', 'onclick' => 'imprimirDetalle();')) !!}
<!-- {!! Form::button('<i class="glyphicon glyphicon-print"></i> Exportar Excel', array('class' => 'btn-sm btn btn-success', 'id' => 'btnDetalle', 'onclick' => 'imprimirDetalleExcel();')) !!}  -->
@if(($tipousuario!=11&&$user->sucursal_id==1)||$user->usertype_id==1 && $sucursal_id == 1)
    <!--{!! Form::button('<i class="glyphicon glyphicon-plus-sign"></i>&nbsp;&nbsp;Cirugías/Procedimientos', array('class' => 'btn-sm btn btn-info', 'id' => 'btncirupro', 'onclick' => 'modal("caja/cirupro", "Cirugías/Procedimientos");')) !!}-->
@endif 
	...
	<tbody>
		...
		<tr style="{{ $color }}" title="{{ $titulo }}">
            ...
            @if($value->conceptopago_id<>2 && $value->conceptopago_id<>1 && $value->situacion<>'A' && !is_null($value->situacion2)  && $value->situacion2<>'R')
                <td align="center">{!! Form::button('<div class="glyphicon glyphicon-print"></div>', array('onclick' => 'imprimirRecibo ('.$value->id.');', 'class' => 'btn-xs btn btn-warning', 'title' => 'Imprimir')) !!}</td>
            ...
            @if($conceptopago_id<>2 && $value->situacion<>'A' && $value->conceptopago_id<>15 && $value->conceptopago_id<>17 && $value->conceptopago_id<>19 && $value->conceptopago_id<>21 && $value->conceptopago_id<>32)
                @if($value->conceptopago_id<>1 && $value->conceptopago_id<>2 && $value->conceptopago_id<>14 && $value->conceptopago_id<>16 && $value->conceptopago_id<>18 && $value->conceptopago_id<>20)
                    @if($user->usertype_id==8 || $user->usertype_id==1 || $user->usertype_id==7 || $user->usertype_id==23 || $user->usertype_id==11 || $user->usertype_id==25)
                        <td align="center">{!! Form::button('<div class="glyphicon glyphicon-remove"></div>', array('onclick' => 'modal (\''.URL::route($ruta["delete"], array($value->id, 'SI')).'\', \''.$titulo_anular.'\', this);', 'class' => 'btn-xs btn btn-danger', 'title' => 'Anular')) !!}</td>
                    @else
                        <td align="center"> - </td>
                    @endif
            ...
            @elseif(($value->conceptopago_id==15 || $value->conceptopago_id==17 || $value->conceptopago_id==19 || $value->conceptopago_id==21|| $value->conceptopago_id==32 || $value->conceptopago_id==100) && $value->situacion=='P')
                <?php //echo 'entro'; ?>
                <td align="center">{!! Form::button('<div class="glyphicon glyphicon-check"></div> Aceptar y Descargar', array('onclick' => 'modal (\''.URL::route($ruta["descarga"], array('movimiento_id'=>$value->id, 'SI')).'\', \'Aceptar\', this);', 'class' => 'btn-xs btn btn-success')) !!}{!! Form::button('<div class="glyphicon glyphicon-check"></div> Aceptar', array('onclick' => 'modal (\''.URL::route($ruta["acept"], array($value->id, 'SI')).'\', \'Aceptar\', this);', 'class' => 'btn-xs btn btn-success')) !!}</td>
                <td align="center">{!! Form::button('<div class="glyphicon glyphicon-remove"></div> Rechazar', array('onclick' => 'modal (\''.URL::route($ruta["reject"], array($value->id, 'SI')).'\', \'Rechazar\', this);', 'class' => 'btn-xs btn btn-danger')) !!}</td>
            @else
                <td align="center"> - </td>
                <td align="center"> - </td>
            @endif         
