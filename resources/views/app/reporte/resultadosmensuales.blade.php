				...
				<div class="box-header">
					<div class="row">
						<div class="col-xs-12">
							...
							{!! Form::button('<i class="glyphicon glyphicon-file"></i> Reporte Excel', array('class' => 'btn btn-success btn-xs', 'id' => 'btnBuscar2', 'onclick' => 'analisisMensualesKtvTru()')) !!}
							{!! Form::close() !!}
						</div>
					</div>
				</div>
...
<script>	
	...
	function analisisMensualesKtvTru(){
        window.open("reporte/analisisMensualesKtvTru?mes="+$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="mes"]').val()+"&anno="+$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="anoo"]').val(),"_blank");
    }
</script>